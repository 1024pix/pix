...
- certains de nos packages ne supportent plus d'anciens navigateurs, ce qui demanderait de gros efforts de notre coté ;
...
- Pix se base aussi sur d'autres sites externes pour valider les compétences numériques, et ces sites ne supportent pas toujours d'anciens navigateurs (en tout cas, nous n'avons aucune vérification de cela)

Donc nous souhaitons supporter les navigateurs qui ont au moins 3 ans, sans supporter de trop vieux navigateurs.
En Aout 2023, en analysant les navigateurs utilisés sur le mois de mai 2023, nous observons que la 5eme version la plus utilisée de Firefox est la version 78 (qui a 3 ans et demi), avec une utilisation globale autour de 0,8%.
Donc si nous voulons gérer les navigateurs de nos utilisateurs avec un peu de marge, une limite à 4 ans est intéressante.

Nous pourrions aussi nous baser uniquement sur des statistiques et les pourcentages d'utilisation mondiales ou français. Ces statistiques sont intéressantes pour s'assurer un bon service pour une population globale. Dans la population des utilisateurs Pix, il y a aussi des utilisateurs très identifiés (des associations, des écoles, etc.) qui ont une gestion qui peut être spécifique de leur parc informatique.
...
De plus, le fichier `/config/target.js` est présents dans les différentes applications front.
...
Pour toutes ces raisons, la solution pour les navigateurs supportés nous semble être celle proposé ci-dessous.

A noter que la valeur par défaut de BrowserList est : `> 0.5%, last 2 versions, Firefox ESR, not dead`
