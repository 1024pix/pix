...
import AlgoResult from "./AlgoResult.js";
...
async function answerTheChallenge({ challenge, allAnswers, allKnowledgeElements, targetSkills, userId, userResult, userKE }) {
  const AnswerStatus = (
  ...
  Answer = (await import("../../api/src/evaluation/domain/models/Answer.js")).Answer;
  ...
  switch (userResult) {
    ...
    case "firstKOthenOK":
      ...
      break;
    case "KE": {
      const ke = userKE.find(userKE, (ke) => challenge.skill.id === ke.skillId);
...
async function _getReferentiel({
  ...
  const dataFetcher = await import(
  ...
  if (campaignId) {
    ...
    const campaignRepository = await import(
      "../../api/lib/infrastructure/repositories/campaign-repository.js"
      );
...
async function _getChallenge({
  ...
  const { pickChallengeService } = await import(
    "../../api/src/certification/flash-certification/domain/services/pick-challenge-service.js"
    );
  const result = smartRandom.getPossibleSkillsForNextChallenge({
    knowledgeElements,
    challenges,
    targetSkills,
    lastAnswer: allAnswers && allAnswers.length ? allAnswers[allAnswers.length - 1] : undefined,
    allAnswers,
    locale
