...
const shouldDisplayStatistics = (statistics) => {
  return statistics.totalParticipantsCount !== null && statistics.totalParticipantsCountByYear !== null;
};

<template>
  {{#if (shouldDisplayStatistics @statistics)}}
  ...
  {{else}}
    <h2>{{t "components.organizations.statistics.missing-data.title"}}</h2>
    <p>{{t "components.organizations.statistics.missing-data.content"}}</p>
  {{/if}}
</template>
