...
export default class SessionsFilterBanner extends Component {
  ...
  <template>
    <PixFilterBanner @title={{t "common.filters.title"}}>
      ...
      >
        <:label><span class="pix-body-l">{{t "pages.sessions.list.filters.startDate.label"}}</span></:label>
      </PixInput>
      ...
      >
        <:label><span class="pix-body-l">{{t "pages.sessions.list.filters.endDate.label"}}</span></:label>
      </PixInput>
