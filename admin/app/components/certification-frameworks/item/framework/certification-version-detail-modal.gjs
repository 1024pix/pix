    ...
    >
      <:content>
        <h2 class="certification-version-detail-modal__subtitle">{{t
            "components.certification-frameworks.item.frameworks.version-detail-modal.parameters"
          }}</h2>
        <PixBlock class="certification-version-detail-modal__parameters">
          <dl>
            <PixIcon @name="star" @ariaHidden={{true}} />
            <dt>{{t "components.certification-frameworks.item.frameworks.version-detail-modal.status"}}</dt>
            <dd>
              <PixTag @color={{get STATUS_COLORS @status}}>
                {{t (concat "components.certification-frameworks.item.frameworks.history.statuses." @status)}}
              </PixTag>
            </dd>

            {{#if @version.startDate}}
              <PixIcon @name="calendar" @ariaHidden={{true}} />
              <dt>{{t "components.certification-frameworks.item.frameworks.version-detail-modal.start-date"}}</dt>
              <dd>{{formatDate @version.startDate}}</dd>
            {{/if}}

            {{#if @version.expirationDate}}
              <PixIcon @name="calendar" @ariaHidden={{true}} />
              <dt>{{t "components.certification-frameworks.item.frameworks.version-detail-modal.expiration-date"}}</dt>
              <dd>{{formatDate @version.expirationDate}}</dd>
            {{/if}}

            <PixIcon @name="flag" @ariaHidden={{true}} />
            <dt>{{t "components.certification-frameworks.item.frameworks.version-detail-modal.locales"}}</dt>
            <dd>
            ...
            <dt>{{t
                "components.certification-frameworks.item.frameworks.version-detail-modal.maximum-assessment-length"
              }}</dt>
            <dd>{{@version.maximumAssessmentLength}}</dd>

            <PixIcon @name="helpSimple" @ariaHidden={{true}} />
            <dt>{{t
                "components.certification-frameworks.item.frameworks.version-detail-modal.minimum-answers-required"
              }}</dt>
