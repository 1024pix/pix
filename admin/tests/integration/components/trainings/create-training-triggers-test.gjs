...
const buildTrainingTrigger = (store) => {
  ...
  return store.createRecord('training-trigger', {
    type: 'goal',
    areas: [area1, area2],
    threshold: 19,
    tubesCount: 5,
...
module('Integration | Component | Trainings::CreateTrainingTriggers', function (hooks) {
  ...
  module("If a goal trigger exists and a prerequisite trigger doesn't exist", function () {
    test('it should display the prerequisite creation link', async function (assert) {
      //given
      const training = {
        goalTrigger: buildTrainingTrigger(this.owner.lookup('service:store')),
      };
  ...
  module("If a prerequisite trigger exists and a goal trigger doesn't exist", function () {
    test('it should display the goal creation link', async function (assert) {
      //given
      const training = {
        prerequisiteTrigger: buildTrainingTrigger(this.owner.lookup('service:store')),
      };
