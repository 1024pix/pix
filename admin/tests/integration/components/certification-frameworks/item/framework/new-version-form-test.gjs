  ...
  test('it should display framework creation form when there is no active version', async function (assert) {
    ...
    assert
      .dom(
        screen.getByRole('button', {
          name: t('components.certification-frameworks.frameworks.new-version-form.submit-button'),
        }),
      )
    ...
    assert.ok(
      screen.getByRole('heading', {
        name: t('components.certification-frameworks.frameworks.new-version-form.title'),
        level: 2,
      }),
    );
    assert
