...
module(
  'Integration | Component | certification-frameworks/certification-framework/versions/certification-version-create-form',
  function (hooks) {
    ...
    test('it should display framework creation form when there is no active version', async function (assert) {
      ...
      // when
      const screen = await render(<template><VersionCreateForm @frameworks={{frameworks}} @scope="DROIT" /></template>);
      ...
      assert
        .dom(
          screen.getByRole('button', {
            name: t('components.certification-frameworks.certification-framework.versions.new.submit-button'),
          }),
        )
    ...
    test('it should pre-select tubes if an active version is loaded along', async function (assert) {
      ...
      const screen = await render(
        <template>
          <VersionCreateForm @frameworks={{frameworks}} @scope="PIX" @activeVersion={{activeVersion}} />
        </template>,
      );
