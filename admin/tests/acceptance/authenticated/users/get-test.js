...
module('Acceptance | authenticated/users/get', function (hooks) {
  ...
  module('when administrator click on anonymize button and confirm modal', function () {
    test('anonymizes the user and redirects to user list', async function (assert) {
      // given
      ...
      assert.dom(screen.getByText(this.intl.t('pages.user-details.overview.anonymisation.success'))).exists();
    });
  });
