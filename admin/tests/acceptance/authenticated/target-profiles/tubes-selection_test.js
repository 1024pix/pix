...
module('Acceptance | Target Profiles | Tubes selection', function (hooks) {
  ...
  module('When admin member is not logged in', function () {
    test('it should not be accessible by an unauthenticated user', async function (assert) {
      // when
      await visit('/target-profiles/tubes-selection');
  ...
  module('When admin member is logged in', function () {
    module('when admin member has role "SUPER_ADMIN", "SUPPORT" or "METIER"', function () {
      test('it should be accessible for an authenticated user', async function (assert) {
        ...
        // then
        assert.strictEqual(currentURL(), '/target-profiles/tubes-selection');
      });
    });
