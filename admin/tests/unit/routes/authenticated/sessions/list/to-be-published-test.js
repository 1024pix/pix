...
module('Unit | Route | authenticated/sessions/list/to-be-published', function(hooks) {
  ...
  module('#model', function() {
    ...
    test('it should return an empty array when fetching from api fails', async function(assert) {
      // given
      const route = this.owner.lookup('route:authenticated/sessions/list/to-be-published');
      sinon.stub(route.store, 'findAll').withArgs('publishable-session').rejects();
