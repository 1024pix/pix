  ...
  createRecord(store, type, snapshot) {
    ...
    if (payload.data.attributes.type === 'COMBINED_COURSE') {
      payload.data.relationships['combined-course-blueprint'] = {
        data: { id: snapshot.record.combinedCourseBlueprint?.id || snapshot.record.course?.sourceId },
      };
      ...
      payload.data.relationships['target-profile'] = {
        data: { id: snapshot.record.targetProfile?.id || snapshot.record.course?.sourceId },
      };
