...
module('Integration | Component | Student::Sco::ManageAuthenticationMethodModal', function (hooks) {
  ...
  module('When Student is not connected with GAR method', function (hooks) {
        ...
        assert
          .dom(
            `button[aria-label="${this.intl.t(
              'pages.students-sco.manage-authentication-method-modal.section.username.copy'
            )}"]`
          )
    ...
    module('When Student is connected with email and username method', function () {
      ...
      test('should render clipboard to copy email', async function (assert) {
        ...
        assert
          .dom(
            `button[aria-label="${this.intl.t(
              'pages.students-sco.manage-authentication-method-modal.section.email.copy'
            )}"]`
          )
    ...
    module('When Student is connected with email only', function () {
      test('should render add username authentication method', async function (assert) {
        // when
        await render(
          hbs`<Student::Sco::ManageAuthenticationMethodModal @student={{this.studentWithEmailOnly}} @display={{this.display}} />`
        );
        ...
        assert.dom('#email').hasValue(email);
        assert.contains(
          this.intl.t('pages.students-sco.manage-authentication-method-modal.section.add-username.label')
        );
      });
    });
    ...
    module('When password is generated', function (hooks) {
      ...
      test('should render clipboard to copy unique password', async function (assert) {
        ...
        assert
          .dom(
            `button[aria-label="${this.intl.t(
              'pages.students-sco.manage-authentication-method-modal.section.password.copy'
            )}"]`
          )
  ...
  module('When Student is connected with GAR method', function (hooks) {
    ...
    test('should render component with GAR connection method', async function (assert) {
      // when
      await render(
        hbs`<Student::Sco::ManageAuthenticationMethodModal @student={{this.studentGAR}} @display={{this.display}} />`
      );
