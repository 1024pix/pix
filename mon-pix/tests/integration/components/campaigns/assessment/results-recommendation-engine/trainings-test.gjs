...
module('Integration | Components | Campaigns | Assessment | ResultsRecommendationEngine | Trainings', function (hooks) {
  ...
  test('it exposes the section as a carrousel, and each card as a labelled slide', async function (assert) {
    ...
    assert.strictEqual(slides[0].getAttribute('aria-roledescription'), 'slide');
    assert.strictEqual(
  ...
  module('training cards a11y', function () {
    ...
    module('when navigations buttons are visible', function (hooks) {
      ...
      test('it sets aria and role attributes for carousel a11y', async function (assert) {
        // given
        const store = this.owner.lookup('service:store');
        const trainings = createManyTrainings(store, 4);
        ...
        assert.strictEqual(region.getAttribute('aria-roledescription'), 'carrousel');
        assert.dom(cards).hasAttribute('aria-live', 'polite');

        assert.strictEqual(slides.length, 3);

        for (let i = 0; i < slides.length; i++) {
          assert.strictEqual(slides[i].getAttribute('aria-roledescription'), 'slide');
          assert.strictEqual(
