...
module('Integration | Component |  Campaigns::Assessment::Tutorial | Steps', function (hooks) {
  ...
  module('on init', function () {
    test('it should display the first step', async function (assert) {
      ...
      assert.dom(screen.getByText(t('pages.tutorial.pages.page0.explanation').replace('\n', ''))).exists();
      ...
      assert.dom(screen.getByRole('button', { name: t('pages.tutorial.next') })).exists();
      assert.dom(screen.getByRole('button', { name: t('pages.tutorial.pass') })).exists();
      assert.dom(screen.queryByRole('button', { name: t('pages.tutorial.start') })).doesNotExist();
    });
  });
  ...
  module('on next step action', function () {
    test('it should display the next step', async function (assert) {
      ...
      // then
      assert.dom(screen.getByText(t('pages.tutorial.pages.page1.title'))).exists();
      assert.dom(screen.getByText(t('pages.tutorial.pages.page1.explanation').replace('\n', ''))).exists();
  ...
  module('on last step', function () {
    test('it should display the last step', async function (assert) {
      ...
      // then
      assert.dom(screen.getByText(t('pages.tutorial.pages.page4.title'))).exists();
      assert.dom(screen.getByText(t('pages.tutorial.pages.page4.explanation').replace('\n', ' '))).exists();
      ...
      assert.dom(screen.queryByRole('button', { name: t('pages.tutorial.next') })).doesNotExist();
      assert.dom(screen.queryByRole('button', { name: t('pages.tutorial.pass') })).doesNotExist();
      assert.dom(screen.getByRole('button', { name: t('pages.tutorial.start') })).exists();
    });
  });
});
