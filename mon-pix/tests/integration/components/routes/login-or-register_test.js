...
describe('Integration | Routes | routes/login-or-register', function () {
  ...
  it('should display the organization name the user is invited to', async () => {
    // when
    await render(
      hbs`{{routes/login-or-register organizationName='Organization Aztec' toggleFormsVisibility=toggleFormsVisibility}}`
    );

    // then
    expect(find('.login-or-register-panel__invitation').textContent).to.equal(
      'Organization Aztec vous invite à rejoindre Pix'
    );
  });
});
