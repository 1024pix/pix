...
module('Integration | Component | Module | Grain', function (hooks) {
  ...
  module('when grain contains a stepper', function () {
    ...
    module('When .shouldFoldStepper is false', function () {
      test('should display every step', async function (assert) {
        // given
        const store = this.owner.lookup('service:store');
        const grain = store.createRecord('grain', {
          title: 'Grain title',
        ...
        // when
        const screen = await render(<template><ModuleGrain @grain={{grain}} @shouldFoldStepper={{false}} /></template>);
