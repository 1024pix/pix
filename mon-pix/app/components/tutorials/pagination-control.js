...
export default class PaginationControl extends Component {
  ...
  changePageSize(event) {
    this.router.replaceWith({ queryParams: { 'page[size]': event.target.value, 'page[number]': 1 } });
  }

  @action
  goToNextPage() {
    this.router.replaceWith({ queryParams: { 'page[number]': this.nextPage } });
  }

  @action
  goToPreviousPage() {
    this.router.replaceWith({ queryParams: { 'page[number]': this.previousPage } });
  }
}
