...
const { host, clientId, afterLogoutUri, endSessionEndpoint } = ENV.neo;

export default class NEOAuthenticator extends BaseAuthenticator {
  @service session;
  @service location;
  ...
  async authenticate({ code, redirectUri, state, authenticationKey }) {
    ...
    if (authenticationKey) {
      serverTokenEndpoint = `${ENV.APP.API_HOST}/api/neo/users?authentication-key=${authenticationKey}`;
      request = {
        method: 'POST',
        headers: {
      ...
      serverTokenEndpoint = `${ENV.APP.API_HOST}/api/neo/token`;
      const bodyObject = {
        code,
        client_id: clientId,
      ...
      // We want to authorize users connected to Pix to connect to Neo as well
      // in order to link their Neo account to the Pix one.
