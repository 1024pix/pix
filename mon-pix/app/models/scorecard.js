...
export default Model.extend({
  ...
  pixScoreAheadOfNextLevelBlocked: computed('earnedPix', 'pixScoreAheadOfNextLevel', function() {
    return this.earnedPix < MAX_REACHABLE_PIX ? this.pixScoreAheadOfNextLevel : 0;
  }),

  earnedPixBlocked: computed('earnedPix', 'pixScoreAheadOfNextLevel', function() {
    return Math.min(this.earnedPix, MAX_REACHABLE_PIX);
  }),

  percentageAheadOfNextLevel: computed('pixScoreAheadOfNextLevelBlocked', function() {
    const percentage = this.pixScoreAheadOfNextLevelBlocked / NUMBER_OF_PIX_BY_LEVEL * 100;
  ...
  remainingPixToNextLevel: computed('pixScoreAheadOfNextLevelBlocked', function() {
    return NUMBER_OF_PIX_BY_LEVEL - this.pixScoreAheadOfNextLevelBlocked;
  }),
