...
module('Acceptance | Download an attachment from a challenge', function (hooks) {
  ...
  module('When the challenge has an attachment', function (hooks) {
    ...
    test('should expose the correct attachment link', function (assert) {
      assert.dom(find('.challenge-statement__action-link').textContent).hasText('Télécharger');
      assert.dom(challengeWithAttachment.attachments.length).to.equal(1);
      assert.equal(
  ...
  module('When the challenge does not contain an attachment', function () {
    hooks.beforeEach(async function () {
      await visit(`/assessments/${assessment.id}/challenges/0`);
      await click('.challenge-actions__action-skip-text');
    });

    test('should hide the download section for the attachment', function () {
      // We are in a challenge...
      assert.dom(find('.challenge-item')).exists();

      // ... but attachment is hidden
      assert.dom(find('.challenge-statement__action-link')).doesNotExist();
    });
  });
});
