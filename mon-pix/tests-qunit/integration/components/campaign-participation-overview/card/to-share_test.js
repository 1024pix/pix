...
module('Integration | Component | CampaignParticipationOverview | Card | ToShare', function (hooks) {
  ...
  test('should render card info when card has "TO_SHARE" status', async function (assert) {
    ...
    assert
      .dom(contains(this.intl.t('pages.campaign-participation-overview.card.tag.completed').toUpperCase()))
      .exists();
    assert.dom(contains(this.intl.t('pages.campaign-participation-overview.card.send'))).exists();
    assert
