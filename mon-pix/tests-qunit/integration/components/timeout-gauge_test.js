...
module('Integration | Component | TimeoutGauge', function (hooks) {
  ...
  module('Component rendering', function () {
    ...
    test('renders a red clock if time is over', async function (assert) {
      ...
      // then
      assert.dom(find(`.timeout-gauge-clock img[src="${RED_GAUGE_ICON_PATH}"]`)).exists();
      assert.dom(find(`.timeout-gauge-clock img[src="${BLACK_GAUGE_ICON_PATH}"]`)).doesNotExist();
    });
    ...
    test('renders a black clock if time is not over', async function (assert) {
      ...
      // then
      assert.dom(find(`.timeout-gauge-clock img[src="${BLACK_GAUGE_ICON_PATH}"]`)).exists();
      assert.dom(find(`.timeout-gauge-clock img[src="${RED_GAUGE_ICON_PATH}"]`)).doesNotExist();
    });
  });
});
