...
module('Unit | Controller | authenticated/sessions/list', function(hooks) {
  ...
  test('it should sort the sessions from more recent to older', function(assert) {
    ...
    const session2 = { id: 2, date: new Date('2018-08-07 14:00:45') };
    const session3 = { id: 3, date: new Date('2018-08-07 14:00:40') };
    ...
    // then
    assert.equal(sortedSessions[0].id, 2);
    assert.equal(sortedSessions[1].id, 1);
    assert.equal(sortedSessions[2].id, 3);
  });
