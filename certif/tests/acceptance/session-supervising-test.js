...
module('Acceptance | Session supervising', function (hooks) {
  ...
  module('when the invigilator handles a live alert', () => {
      ...
      module('when supervisor dismisses a live alert', function () {
        ...
        module('when there is an error', function () {
          test('it displays an error notification', async function (assert) {
            ...
            server.patch(
              `/sessions/${sessionId}/candidates/${candidateId}/dismiss-live-alert`,
              () => new Response(400),
            );
            this.sessionForSupervising = server.create('session-for-supervising', {
              id: sessionId,
    ...
    module('when this is a companion live alert', function () {
      module('when the invigilator cleared the alert', function () {
        module('when there is no error', function () {
          test('it cleared the live alert and displays a success notification', async function (assert) {
            // given
            const sessionId = 12345;
            const candidateUserId = 123;
            this.sessionForSupervising = server.create('session-for-supervising', {
              id: sessionId,
              certificationCandidates: [
                server.create('certification-candidate-for-supervising', {
            ...
            await screen.findByRole('dialog');
            await click(
              screen.getByRole('button', { name: 'Confirmer la présence de l’extension et traiter le signalement' }),
            );
            await settled();
            ...
            assert
              .dom(
                screen.getByText(
                  'Le signalement a bien été traité. Veuillez indiquer au candidat de rafraîchir sa page',
                  { exact: false },
                ),
              )
              .exists();
        ...
        module('when there is an error', function () {
          test('it displays an error notification', async function (assert) {
            ...
            server.patch(
              `/sessions/${sessionId}/users/${candidateUserId}/clear-companion-alert`,
              () => new Response(400),
            );
