...
module.exports = {
  ...
  async parseCertificationCandidatesFromAttendanceSheet(request) {
    const userId = request.auth.credentials.userId;
    const sessionId = request.params.id;
    const odsBuffer = request.payload.file;

    const certificationCandidates = await usecases.parseCertificationCandidatesFromAttendanceSheet(
      {
        userId,
        sessionId,
        odsBuffer
