...
class OidcAuthenticationService {
  ...
  constructor(
    ...
    const isEnabledInPixAdmin = config[this.configKey].isEnabledInPixAdmin;
    if (!isEnabledInConfig && !isEnabledInPixAdmin) {
      return;
    }
    ...
    this.#isReady = true;
    this.#isReadyForPixAdmin = isEnabledInPixAdmin;
  }
  ...
  async getUserInfo({ idToken, accessToken }) {
    const { family_name, given_name, sub, nonce, email } = jsonwebtoken.decode(idToken);
    let userInfoContent;
