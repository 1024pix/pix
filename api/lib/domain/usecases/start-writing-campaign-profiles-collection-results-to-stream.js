...
module.exports = async function startWritingCampaignProfilesCollectionResultsToStream(
  ...
  // complete operation.
  const campaignParticipationResultDataChunks = _.chunk(campaignParticipationResultDatas, 20);
  bluebird.map(campaignParticipationResultDataChunks, async (campaignParticipationResultDataChunk) => {
    ...
    const certificationProfiles = await certificationProfileService.getCertificationProfiles({
      userIdsAndDates,
