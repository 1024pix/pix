...
module.exports = async function getAnswerWithRecentKnowledgeElements({ answerId, answerRepository, assessmentRepository, smartPlacementKnowledgeElementRepository }) {
  ...
  if(answer.knowledgeElements.length > 0) {
    ...
    if(assessment.isSmartPlacement()) {
      const firstKnowledgeElementsKnownBySkillIds = await smartPlacementKnowledgeElementRepository.findFirstSavedKnowledgeElementsByUserId({
        userId: assessment.userId,
        skillIds: _.map(answer.knowledgeElements, 'skillId')
      });
      const knowledgeElementsToKeep = _.intersectionBy(firstKnowledgeElementsKnownBySkillIds, answer.knowledgeElements, 'id');
