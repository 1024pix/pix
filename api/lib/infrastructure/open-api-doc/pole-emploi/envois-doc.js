...
const campagne = Joi.object({
  ...
  // idCampagne: Joi.number().required().example(12345).description('Identifiant de la campagne'),
...
const evaluation = Joi.object({
  ...
  uniteScore: Joi.string().valid(...UNITS).example('A').description('Unite de score. A: Pourcentage, B: Point'),
  certifiable: Joi.boolean().example(true).description('Uniquement pour les campagnes de collecte profils. Vide si pas encore partagé'),
  niveau: Joi.number().min(1).max(8).example(1).description('Pour campagne de collecte de profil, niveau de la compétence (entre 1 et 8). Vide si pas encore partagé'),
...
const elementEvalue = Joi.object({
  libelle: Joi.string().max(255).required().example('Mener une recherche et une veille d’information').description('Libellé compétence évaluée'),
  categorie: Joi.string().required().max(50).example('competence').description('Catégorie de l\'élément évalué'),
  type: Joi.string().required().max(50).example('competence Pix').description('Type de l\'élément évalué'),
  domaineRattachement: Joi.string().required().max(255).example('1. Information et données').description('Domaine de rattachement de la compétence évaluée'),
  nbSousElements: Joi.number().example(2).description('Pour campagne d\'évaluation uniquement. Nombre d’acquis total pour la compétence évaluée'),
  evaluation,
});
...
const test = Joi.object({
  ...
  evaluation: Joi.number().allow(null).example(3.14).description('Pour une campagne d\'évaluation, pourcentage de maitrise de l\'ensemble des acquis du profil. Pour une campagne de collecte de profil, score du participant. Vide si pas encore partagé'),
  uniteEvaluation: Joi.string().valid(...UNITS).example('A').description('Unite d\'évaluation. A: Pourcentage, B: Point'),
