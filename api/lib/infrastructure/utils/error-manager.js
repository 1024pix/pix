...
function _mapToInfrastructureError(error) {
  ...
  if (error instanceof DomainErrors.CertificationCandidateCreationOrUpdateError) {
    return new InfraErrors.BadRequestError('Une erreur s\'est produit lors de la création ou de la mise à jour du candidat de certification');
  }
  if (error instanceof DomainErrors.CertificationCandidateDeletionError) {
    return new InfraErrors.BadRequestError('Une erreur s\'est produit lors de la suppression du candidat de certification');
  }
