...
async function _getResultListPaginated(campaignId, targetProfile, filters, page) {
  const query = _getParticipantActivityResultList(campaignId, targetProfile, filters);
  return fetchPage(query, page);
}

function _getParticipantActivityResultList(campaignId, targetProfile, filters) {
