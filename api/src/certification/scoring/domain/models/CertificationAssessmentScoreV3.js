...
const _computeScore = ({
  ...
  let _normalizedCapacity = capacity;
  ...
  if (_normalizedCapacity < minimumCapacity) {
    _normalizedCapacity = minimumCapacity;
  }
  if (_normalizedCapacity > maximumCapacity) {
    _normalizedCapacity = maximumCapacity;
  }

  const intervalIndex = _findIntervalIndex(_normalizedCapacity, certificationScoringIntervals);
  ...
  const score = intervalHeight * (intervalIndex + 1 + (_normalizedCapacity - intervalMaxValue) / intervalWidth);
