...
export async function editCandidate({
  ...
  if (!session.canEditEnrolledCandidate(candidate.id, enrolledCandidates)) {
  ...
  const otherEnrolledCandidates = enrolledCandidates.filter(
    (enrolledCandidate) => candidate.id !== enrolledCandidate.id,
  );
  const isAlreadyEnrolled = session.isCandidateAlreadyEnrolled({
    enrolledCandidates: otherEnrolledCandidates,
