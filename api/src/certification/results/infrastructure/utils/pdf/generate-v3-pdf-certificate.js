...
export function testpdfkit() {
  ...
  for (const area_data of Object.values(areas_text)) {
    ...
    let docWithStyle = docWithStyleForAreaTitle(doc);
    ...
    for (const { label, content } of Object.values(area_data.competences_text)) {
      ...
      docWithStyle.text(label, { width: columnWidth });
      addGap(doc, init_y);
...
function docWithStyleForAreaTitle(pdfDoc) {
  return pdfDoc.font('Nunito-Bold').fillColor('#F0F').fontSize(AREA_FONT_SIZE);
}
...
function docWithStyleForParagraphText(pdfDoc) {
  return pdfDoc.font('Roboto-Regular').fillColor('#000000').fontSize(7);
}
...
function writeParagraph(pdfDoc, text, columnWidth, columnGap, initY) {
  ...
  pdfDoc.text(textQuiRentre, { width: columnWidth });
  if (textQuiReste.length > 0) {
    changeColumn(pdfDoc, columnWidth, columnGap, initY);
    pdfDoc.text(textQuiReste, { width: columnWidth });
  }
}
