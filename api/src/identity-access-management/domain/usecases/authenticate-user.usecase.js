...
const authenticateUser = async function ({
  ...
  try {
    ...
    if (isSessionLogoutEnabled) {
      encodedRefreshToken = UserRefreshToken.generate({ userId: user.id, audience, sessionId, source });
    } else {
      const refreshToken = RefreshToken.generate({ userId: user.id, audience, sessionId, source });
