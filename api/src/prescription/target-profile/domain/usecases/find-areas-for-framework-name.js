...
export async function findAreasForFrameworkName({
  frameworkName,
  locale,
  skillRepository,
  tubeRepository,
  ...
  const framework = await frameworkRepository.getByName({ name: frameworkName });
  const areasWithCompetences = await areaRepository.findByFrameworkIdWithCompetences({
