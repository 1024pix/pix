...
export async function findValidatedByCompetenceId(competenceId, locale) {
  const lcmsChallenges = await challengeRepository.findValidatedByCompetenceId(competenceId, locale);
...
export async function findOperativeBySkillsAndLocales(skills, locales) {
  const lcmsChallenges = await challengeRepository.findOperativeBySkillsAndLocales(skills, locales);
  return lcmsChallenges.map(
    (lcmsChallenge) =>
      new SmartRandomChallenge({
        id: lcmsChallenge.id,
