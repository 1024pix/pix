...
async function _computeCampaignParticipationResults(campaign) {
  ...
  const participantsResults = await bluebird.mapSeries(campaignParticipationInfosChunks, computeResultsWithTargetedSkillIds);
...
async function _getKnowledgeElementsByUser(campaignParticipations) {
  ...
  const knowledgeElementByUser = await knowlegeElementSnapshotRepository.findByUserIdsAndSnappedAtDates(sharingDateByUserId);
...
if (require.main === module) {
  computeParticipantResultsShared(concurrency)
    .then(handleSuccess)
    .catch(handleError)
    .finally(exit);
}
