  ...
  describe('POST /api/answers', function () {
    ...
    context('when challenge is a QROCM-dep type', function () {
      context('when the answer is valid', function () {
        ...
        it('should return expected resultDetails', async function () {
          // when
          const response = await promise;

          // then
          expect(response.result.data.attributes['result-details']).to.deep.equal('null\n');
        });
      });
      ...
      context('when the answer is false', function () {
        ...
        it('should return expected resultDetails', async function () {
          ...
          // then
          expect(response.result.data.attributes['result-details']).to.deep.equal(
            'answersEvaluation:\n' +
              '  - true\n' +
              '  - false\n' +
              '  - false\n' +
