...
describe('Integration | UseCase | compute-campaign-analysis', () => {
  ...
  beforeEach(() => {
    ...
    knowledgeElementRepository = { findByUserIdsAndDatesGroupedByUserIdAndCompetenceIdWithSnapshotting: sinon.stub() };
  ...
  context('User has access to this result', () => {
    it('should returns two CampaignTubeRecommendations with computed average score', async () => {
      ...
      knowledgeElementRepository.findByUserIdsAndDatesGroupedByUserIdAndCompetenceIdWithSnapshotting
        .withArgs({ 1: 'someDate' })
        .resolves({ 1: { [competence.id]: [knowledgeElementSkill1,knowledgeElementSkill2, knowledgeElementSkill3] } });
