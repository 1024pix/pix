...
describe('Integration | Services | extractCertificationsDataFromAttendanceSheet', () => {
  ...
  context('When attendance sheet is of version 1.2', () => {
    ...
    context('when the data contains rows with no last name', () => {
      ...
      it('should return the data without the rows with no last name', async () => {
        // given
        const odsFilePath = `${__dirname}/attendance_sheet_1-2_no_last_name_ok_test.ods`;
        const odsBuffer = fs.readFileSync(odsFilePath);
        const expectedData = [
          { lastName: 'Lantier',
            firstName: 'Étienne',
    ...
    context('when data is regular', () => {
      ...
      it('should return expected data', async () => {
        // given
        const odsFilePath = `${__dirname}/attendance_sheet_1-2_ok_test.ods`;
        const odsBuffer = fs.readFileSync(odsFilePath);
        const expectedData = [{
          lastName: 'Baudu',
          firstName: null,
      ...
      it('should trim whitespaces in names', async () => {
        // given
        const odsFilePath = `${__dirname}/attendance_sheet_1-2_spaces_around_names_test.ods`;
        const odsBuffer = fs.readFileSync(odsFilePath);
        const expectedData = [{
          lastName: 'EspaceAvant',
          firstName: 'EspaceAvant',
