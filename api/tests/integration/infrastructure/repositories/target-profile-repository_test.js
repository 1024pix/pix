...
describe('Integration | Repository | Target-profile', () => {
  ...
  describe('#findTargetProfilesSharedWithOrganization', () => {
    ...
    beforeEach(async () => {
      theRequestedOrganization = databaseBuilder.factory.buildOrganization(theRequestedOrganization);
      anotherOrganization = databaseBuilder.factory.buildOrganization(anotherOrganization);
      targetProfileSkillAssociation = databaseBuilder.factory.buildTargetProfilesSkills({
        targetProfileId: anotherOrganization.id,
        isPublic: false
      });
      ...
      anotherOrganizationTargetProfile = databaseBuilder.factory.buildTargetProfile(anotherOrganizationTargetProfile);
      databaseBuilder.factory.buildTargetProfilesShared({
        organizationId: theRequestedOrganization.id,
        targetProfileId: anotherOrganizationTargetProfile.id
      });
    ...
    it('should return an Array', () => {
      // when
      const promise = targetProfileRepository.findTargetProfilesSharedWithOrganization(theRequestedOrganization.id);
    ...
    it('should return target profiles shared with the organization', () => {
      // when
      const promise = targetProfileRepository.findTargetProfilesSharedWithOrganization(theRequestedOrganization.id);
      ...
      return promise.then((foundTargetProfiles) => {
        expect(foundTargetProfiles).to.have.lengthOf(1);
        expect(foundTargetProfiles[0]).to.be.an.instanceOf(TargetProfile);
        expect(foundTargetProfiles[0].organizationId).to.not.equal(theRequestedOrganization.id);
        expect(foundTargetProfiles[0].id).to.equal(anotherOrganizationTargetProfile.id);
      });
    });

    it('should contains skills linked to every target profiles', () => {
      // when
      const promise = targetProfileRepository.findTargetProfilesSharedWithOrganization(theRequestedOrganization.id);
