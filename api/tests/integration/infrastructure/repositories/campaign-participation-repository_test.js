...
describe('Integration | Repository | Campaign Participation', () => {
  ...
  describe('#findSharedParticipationsWithUserIdsAndDates', () => {

    it('should return an object where keys are userIds and values are shared dates for a campaign', async () => {
      // given
      const campaignId = databaseBuilder.factory.buildCampaign().id;
      const sharedParticipation1 = databaseBuilder.factory.buildCampaignParticipation({ campaignId, isShared: true });
      const sharedParticipation2 = databaseBuilder.factory.buildCampaignParticipation({ campaignId, isShared: true });
      const sharedParticipation3 = databaseBuilder.factory.buildCampaignParticipation({ campaignId, isShared: true });
      await databaseBuilder.commit();

      // when
      const userIdsAndDates = await campaignParticipationRepository.findSharedParticipationsWithUserIdsAndDates(campaignId);

      expect(userIdsAndDates).to.deep.equal({
        [sharedParticipation1.userId]: sharedParticipation1.sharedAt,
        [sharedParticipation2.userId]: sharedParticipation2.sharedAt,
        [sharedParticipation3.userId]: sharedParticipation3.sharedAt,
      });
    });

    it('should ignore participations not in campaign', async () => {
      // given
      const campaignId = databaseBuilder.factory.buildCampaign().id;
      const anotherCampaignId = databaseBuilder.factory.buildCampaign().id;
      const sharedParticipation = databaseBuilder.factory.buildCampaignParticipation({ campaignId, isShared: true });
      databaseBuilder.factory.buildCampaignParticipation({ campaignId: anotherCampaignId, isShared: true });
      await databaseBuilder.commit();
      ...
      expect(userIdsAndDates).to.deep.equal({
        [sharedParticipation.userId]: sharedParticipation.sharedAt,
      });
    });

    it('should ignore participations not shared', async () => {
      // given
      const campaignId = databaseBuilder.factory.buildCampaign().id;
      const sharedParticipation = databaseBuilder.factory.buildCampaignParticipation({ campaignId, isShared: true });
      databaseBuilder.factory.buildCampaignParticipation({ campaignId, isShared: false });
    ...
    it('should return an empty object if no participations are shared in campaign', async () => {
      // given
      const campaignId = databaseBuilder.factory.buildCampaign().id;
      databaseBuilder.factory.buildCampaignParticipation({ campaignId, isShared: false });
      await databaseBuilder.commit();
