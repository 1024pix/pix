...
describe('Integration | Repository | Badge Acquisition', () => {
  ...
  describe('#getCertifiableAcquiredBadges', () => {
    let badgeCertifiable1, badgeNonCertifiable, user, userWithoutBadge;
    beforeEach(async () => {
      const targetProfile = databaseBuilder.factory.buildTargetProfile();
      badgeCertifiable1 = databaseBuilder.factory.buildBadge({ targetProfileId: targetProfile.id, isCertifiable: true });
      databaseBuilder.factory.buildBadge({ targetProfileId: targetProfile.id, isCertifiable: true });
      badgeNonCertifiable = databaseBuilder.factory.buildBadge({ targetProfileId: targetProfile.id, isCertifiable: false });
    ...
    it('should return badges certifiables acquired by users', async () => {
      // when
      const certifiableBadgesAcquiredByUser = await badgeAcquisitionRepository.getCertifiableAcquiredBadges({
        userId: user.id,
      });
