  ...
  describe('#findPaginatedByCampaignId', function() {
    ...
    context('when there is a filter on stage', function() {
      ...
      it('returns participants which have the given stage', async function() {
        ...
        databaseBuilder.factory.buildAssessmentFromParticipation({ masteryRate: 0, participantExternalId: 'Juste Before', campaignId: campaign.id }, { id: 1 });
        databaseBuilder.factory.buildAssessmentFromParticipation({ masteryRate: 0.25, participantExternalId: 'Stage Reached Boundary IN', campaignId: campaign.id }, { id: 2 });
        databaseBuilder.factory.buildAssessmentFromParticipation({ masteryRate: 0.74, participantExternalId: 'Stage Reached Boundary OUT', campaignId: campaign.id }, { id: 3 });
        databaseBuilder.factory.buildAssessmentFromParticipation({ masteryRate: 0.75, participantExternalId: 'Just After', campaignId: campaign.id }, { id: 4 });
        await databaseBuilder.commit();
      ...
      it('returns participants which have validated skill count between several boundaries', async function() {
        ...
        databaseBuilder.factory.buildAssessmentFromParticipation({ masteryRate: 0, participantExternalId: 'The good', campaignId: campaign.id });
        databaseBuilder.factory.buildAssessmentFromParticipation({ masteryRate: 0.33, participantExternalId: 'The bad', campaignId: campaign.id });
        databaseBuilder.factory.buildAssessmentFromParticipation({ masteryRate: 0.66, participantExternalId: 'The ugly', campaignId: campaign.id });
        await databaseBuilder.commit();
