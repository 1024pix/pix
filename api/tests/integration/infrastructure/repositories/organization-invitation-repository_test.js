...
describe('Integration | Repository | OrganizationInvitationRepository', () => {
  ...
  describe('#create', () => {
    ...
    it('should save the organization invitation in db', async () => {
      ...
      expect(savedInvitation.temporaryKey).equal(temporaryKey);
    });
  });
  ...
  describe('#get', () => {
    ...
    it('should get the organization-invitation from db', async () => {
      ...
      expect(foundOrganizationInvitation.temporaryKey).to.equal(insertedOrganizationInvitation.temporaryKey);
    });
  ...
  describe('#getByIdAndTemporaryKey', () => {
    ...
    it('should get the organization-invitation by id and temporaryKey', async () => {
      // given
      const { id, temporaryKey } = insertedOrganizationInvitation;

      // when
      const foundOrganizationInvitation = await organizationInvitationRepository.getByIdAndTemporaryKey({ id, temporaryKey });
    ...
    it('should return a rejection when organization-invitation id and temporaryKey are not found', async () => {
      // given
      const nonExistentId = 10083;
      const nonExistentTemporaryKey = 'ABCDEF';

      // when
      const requestErr = await catchErr(organizationInvitationRepository.getByIdAndTemporaryKey)({
        id: nonExistentId,
        temporaryKey: nonExistentTemporaryKey
      });
  ...
  describe('#markAsAccepted', () => {
    ...
    it('should update model in database', async () => {
      ...
      expect(organizationInvitationSaved.temporaryKey).to.equal(organizationInvitation.temporaryKey);
    });
  });
