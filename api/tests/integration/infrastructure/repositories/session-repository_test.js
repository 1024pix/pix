...
describe('Integration | Repository | Session', function() {
  ...
  describe('#get', () => {
    ...
    beforeEach(async () => {
      ...
      _.times(2, () => databaseBuilder.factory.buildCertificationCourse({ sessionId }));
    ...
    it('should return session informations in a session Object', async () => {
      // when
      const actualSession = await sessionRepository.get(sessionId);

      // then
      expect(actualSession).to.be.instanceOf(Session);
      expect(actualSession.id).to.be.equal(sessionId);
      expect(actualSession.certificationCenter).to.be.equal('Tour Gamma');
      expect(actualSession.address).to.be.equal('rue de Bercy');
      expect(actualSession.room).to.be.equal('Salle A');
      expect(actualSession.examiner).to.be.equal('Monsieur Examinateur');
      expect(actualSession.date).to.be.equal('2018-02-23');
      expect(actualSession.time).to.be.equal('12:00');
      expect(actualSession.description).to.be.equal('CertificationPix pour les jeunes');
      expect(actualSession.accessCode).to.be.equal('NJR10');
    });
    ...
    it('should return associated certifications', async () => {
      ...
      // then
      expect(actualSession.certifications).to.be.instanceOf(Array);
      expect(actualSession.certifications.length).to.be.equal(2);
    });

    it('should return a Not found error when no session was found', async () => {
      // when
      try {
        await sessionRepository.get(2);
      } catch (error) {
        // then
  ...
  describe('#getWithCertificationCandidates', () => {
    ...
    beforeEach(async () => {
      ...
      databaseBuilder.factory.buildCertificationCandidate({ lastName: 'Jackson', firstName: 'Michael', sessionId });
      databaseBuilder.factory.buildCertificationCandidate({ lastName: 'Stardust', firstName: 'Ziggy', sessionId });
      databaseBuilder.factory.buildCertificationCandidate({ lastName: 'Jackson', firstName: 'Janet', sessionId });
    ...
    it('should return session informations in a session Object', async () => {
      // when
      const actualSession = await sessionRepository.getWithCertificationCandidates(sessionId);
    ...
    it('should return associated certifications candidates ordered by lastname and firstname', async () => {
      ...
      // then
      expect(actualSession.certificationCandidates).to.be.instanceOf(Array);
      expect(actualSession.certificationCandidates.length).to.be.equal(3);
      _.each(actualSession.certificationCandidates, (candidate) => {
        expect(candidate).to.have.property('sessionId', sessionId);
      });
      expect(actualSession.certificationCandidates[0].lastName).to.equal('Jackson');
      expect(actualSession.certificationCandidates[0].firstName).to.equal('Janet');
      expect(actualSession.certificationCandidates[1].lastName).to.equal('Jackson');
      expect(actualSession.certificationCandidates[1].firstName).to.equal('Michael');
      expect(actualSession.certificationCandidates[2].lastName).to.equal('Stardust');
      expect(actualSession.certificationCandidates[2].firstName).to.equal('Ziggy');
    });

    it('should return a Not found error when no session was found', async () => {
      // when
      try {
        await sessionRepository.get(sessionId + 1);
      } catch (error) {
        // then
        return expect(error).to.be.instanceOf(NotFoundError);
      }
