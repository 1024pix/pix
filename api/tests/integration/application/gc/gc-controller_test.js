...
describe('Integration | Application | GC | gc-controller', () => {
  ...
  beforeEach(() => {
    ...
    sinon.stub(gcController, 'generateAndDownloadHeapProfile').resolves('any Node.js Stream');
  ...
  describe('#generateAndDownloadHeapProfile', () => {
    ...
    context('Success cases', () => {
      ...
      it('should resolve a 200 HTTP response', async () => {
        // when
        const response = await httpTestServer.request('GET', '/api/gc/heap-profile');
    ...
    context('Error cases', () => {
      ...
      context('when user is not allowed to access resource', () => {
        ...
        it('should resolve a 403 HTTP response', () => {
          // when
          const promise = httpTestServer.request('GET', '/api/gc/heap-profile');
