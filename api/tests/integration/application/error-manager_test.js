...
describe('Integration | API | Controller Error', () => {
  ...
  context('412 Precondition Failed', () => {
    ...
    it('responds Precondition Failed when a NoCampaignParticipationForUserAndCampaign error occurs', async () => {
  ...
  context('409 Conflict', () => {
    ...
    it('responds Conflict when a AccountRecoveryUserAlreadyConfirmEmail error occurs', async () => {
      ...
      expect(response.statusCode).to.equal(CONFLICT_ERROR);
      expect(responseDetail(response)).to.equal('This user had already a confirmed email.');
    });
