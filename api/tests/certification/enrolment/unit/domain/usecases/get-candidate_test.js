...
describe('Certification | Enrolment | Unit | UseCase | get-candidate', function () {
  ...
  it('should return the candidate by id', async function () {
    ...
    // when
    const result = await getCandidate({ certificationCandidateId: 1234, candidateRepository, certificationBadgesService, certificationCenterRepository });
  ...
  context('when the candidate is not registered to double certification', function () {
    it('should set doubleCertificationEligibility to false', async function () {
      ...
      // when
      const result = await getCandidate({ certificationCandidateId: candidate.id, candidateRepository, certificationBadgesService, certificationCenterRepository });
  ...
  context('when the candidate is registered to double certification', function () {
    context('when the center is not habilitated', function () {
      it('should set doubleCertificationEligibility to false', async function () {
        // given
        const candidate = domainBuilder.certification.enrolment.buildCandidate({ subscription: Frameworks.CLEA, userId: 456, sessionId: 789, reconciledAt: new Date() });
    ...
    context('when the center is habilitated', function () {
      context('when the candidate has no valid badge', function () {
        it('should set doubleCertificationEligibility to false', async function () {
          ...
          const candidate = domainBuilder.certification.enrolment.buildCandidate({ subscription: Frameworks.CLEA, userId: 456, sessionId: 789, reconciledAt });
          const habilitation = domainBuilder.certification.shared.buildComplementaryCertification({ key: Frameworks.CLEA });
          const center = domainBuilder.certification.enrolment.buildCenter({ habilitations: [habilitation] });
          candidateRepository.get.resolves(candidate);
          certificationCenterRepository.getBySessionId.withArgs({ sessionId: 789 }).resolves(center);
          certificationBadgesService.findStillValidBadgeAcquisitions.withArgs({ userId: 456, limitDate: reconciledAt }).resolves([]);
      ...
      context('when the candidate has a valid badge matching their subscription', function () {
        it('should set doubleCertificationEligibility to true', async function () {
          ...
          const habilitation = domainBuilder.certification.shared.buildComplementaryCertification({ key: Frameworks.CLEA });
          const center = domainBuilder.certification.enrolment.buildCenter({ habilitations: [habilitation] });
          const badgeAcquisition = domainBuilder.buildCertifiableBadgeAcquisition({ complementaryCertificationKey: Frameworks.CLEA });
          candidateRepository.get.resolves(candidate);
          certificationCenterRepository.getBySessionId.withArgs({ sessionId: 789 }).resolves(center);
          certificationBadgesService.findStillValidBadgeAcquisitions.withArgs({ userId: 456, limitDate: reconciledAt }).resolves([badgeAcquisition]);
