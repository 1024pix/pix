...
describe('Unit | Certification | Enrolment | Domain | Models | Session', function () {
  ...
  context('#isPublished', function () {
    it('returns true when the session is published', function () {
      // given
      const session = domainBuilder.certification.enrolment.buildSession({ publishedAt: new Date() });
      // when
    ...
    it('returns false when the session is not published', function () {
      // given
      const session = domainBuilder.certification.enrolment.buildSession({ publishedAt: null });
      // when
      const isPublished = session.isPublished();
