    ...
    context('when user has role Super Admin', function () {
      ...
      context('when the session exists', function () {
        context('when the certification course contains challenges', function () {
          it('should return a 204 status code and post publishSession event', async function () {
            ...
            await expect(handler.jobName).to.have.performed.withJobsCount(1);
          });
        });

        context('when the certification course does not contain challenges', function () {
          let certificationId, sessionId;
          beforeEach(function () {
