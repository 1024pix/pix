...
describe('Integration | Application | campaign-controller', function () {
  describe('#getPresentationSteps', function () {
    it('returns organizations groups', async function () {
      ...
      sinon.stub(usecases, 'getPresentationSteps').withArgs({ campaignId: campaign.id, locale: 'fr-fr' }).resolves({
      ...
      const result = await campaignController.getPresentationSteps(request, hFake);
