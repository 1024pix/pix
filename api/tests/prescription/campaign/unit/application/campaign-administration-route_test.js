...
describe('Unit | Application | Router | campaign-administration-router', function () {
  describe('POST /api/campaigns', function () {
    it('should not call controller method when security prehandler failed', async function () {
      // given
      sinon.stub(securityPreHandlers, 'checkOrganizationAccess').throws();
      sinon.stub(campaignAdministrationController, 'save');
      const httpTestServer = new HttpTestServer();
      await httpTestServer.register(moduleUnderTest);

      // when
      await httpTestServer.request('POST', '/api/campaigns');

      // then
      expect(campaignAdministrationController.save.called).to.be.false;
    });
  });
