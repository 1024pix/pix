  ...
  describe('#isEligible', function () {
    describe('edge cases', function () {
      it('make it work with targetProfileIds', function () {
        ...
        expect(quest.isEligible(eligibilityData)).to.be.true;
      });

      it('should return false when criteria are valid but not in the same object', function () {
        const quest = new Quest({
          eligibilityRequirements: [
            {
              type: TYPES.CAMPAIGN_PARTICIPATIONS, // Array
        ...
        const eligibilityData = new Eligibility({
          campaignParticipations: [
            { targetProfileId: 2, status: 'SHARED' },
            { targetProfileId: 1, status: 'STARTED' },
          ],
        });

        expect(quest.isEligible(eligibilityData)).to.be.false;
      });
    });
  });
    ...
    before(function () {
      ...
      const eligibilityRequirements = [
        ...
        },
        {
          type: TYPES.CAMPAIGN_PARTICIPATIONS, // Array
          data: {
            targetProfileId: [eligibleTargetProfileId],
          },
          comparison: COMPARISON.ALL,
        },
      ];
