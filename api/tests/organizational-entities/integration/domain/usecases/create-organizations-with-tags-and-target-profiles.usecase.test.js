...
describe('Integration | UseCases | create-organizations-with-tags-and-target-profiles', function () {
      ...
      it('throws an error', async function () {
        ...
        expect(error.invalidAttributes).to.eql([
          ...
          },
          {
            attribute: 'locale',
            message:
              "La locale doit avoir l'une des valeurs suivantes : de-at, en, es, es-419, fr, nl, fr-be, fr-fr, nl-be, it",
          },
          {
            attribute: 'locale',
  ...
  describe('when one provided category id is not found in database', function () {
    it('should rollback', async function () {
      ...
      const organizationsWithOneNonExistingCategoryId = [
        {
          type: 'SCO',
          externalId: 'a100',
          name: 'Collège Bob',
      ...
      // when
      const error = await catchErr(usecases.createOrganizationsWithTagsAndTargetProfiles)({
        organizations: organizationsWithOneNonExistingCategoryId,
      });
