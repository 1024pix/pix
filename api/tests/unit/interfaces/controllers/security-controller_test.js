...
describe('Unit | Interfaces | Controllers | SecurityController', () => {
  ...
  describe('#checkUserIsAdminInMembershipOrganization', () => {
    let isAdminInOrganizationStub;
    ...
    context('Successful case', () => {
      const request = {
        ...
        payload: { data: { relationships: { organization: { data : { id: 5678 } } } } }
      };
      ...
      it('should authorize access to resource when the user is authenticated and is ADMIN in membership Organization', async () => {
        // given

        // when
        const response = await securityController.checkUserIsAdminInMembershipOrganization(request, hFake);
    ...
    context('Error cases', () => {
      ...
      it('should forbid resource access when user is not ADMIN in membership Organization', async () => {
        // given
        checkUserIsAdminInOrganizationUseCase.execute.resolves(false);
