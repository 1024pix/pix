...
describe('Unit | Domain | Service | Certifiable Badges', function() {
  ...
  beforeEach(async () => {
    ...
    // given
    const getCertifiableAcquiredBadgesStub = sinon.stub(badgeAcquisitionRepository, 'getCertifiableAcquiredBadges');
    getCertifiableAcquiredBadgesStub.withArgs({ userId: userIdWithoutCertifiableBadges }).resolves([]);
    getCertifiableAcquiredBadgesStub.withArgs({ userId: userIdWithCertifiableBadges }).resolves([badgeAcquisition1, badgeAcquisition2, badgeAcquisition3]);
  });
