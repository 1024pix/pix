...
describe('Unit | Application | UseCase | authenticate-user', function () {
  ...
  context('check acces by pix scope', function () {
    ...
    context('when scope is pix-admin', function () {
      it('should throw an error when user has no role so he is not an admin member', async function () {
        // given
      ...
      it('should resolves a valid JWT access token when admin member is not disabled and have a valid role so authentication succeeded', async function () {
        // given
        const scope = appMessages.PIX_ADMIN.SCOPE;
        const source = 'pix';
