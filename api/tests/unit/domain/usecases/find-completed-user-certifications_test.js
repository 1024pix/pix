...
describe('Unit | UseCase | find-completed-user-certifications', () => {
  ...
  beforeEach(() => {
    certificationRepository.findCompletedCertificationsByUserId = sinon.stub();
  });
  ...
  it('should return all the needed informations about certifications', function() {
    ...
    certificationRepository.findCompletedCertificationsByUserId.resolves([certification]);
    ...
    return promise.then((certifications) => {
      expect(certificationRepository.findCompletedCertificationsByUserId).to.have.been.calledWith(userId);
