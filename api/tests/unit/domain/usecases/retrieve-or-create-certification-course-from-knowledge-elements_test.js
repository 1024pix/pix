...
describe('Unit | UseCase | retrieve-or-create-certification-course-from-knowledge-elements', () => {
  ...
  describe('#retrieveOrCreateCertificationCourseFromKnowledgeElements', () => {
    ...
    it('should create the certification course and add challenges', async function() {
      ...
      sinon.stub(userService, 'getUserCertificationChallenges')
        .withArgs(userId, sinon.match.any)
        .resolves(userCompetencesAndChallenges);
      ...
      // when
      const newCertification = await retrieveOrCreateCertificationCourseFromKnowledgeElements({ userId, sessionId, userService, certificationChallengesService, certificationCourseRepository });
