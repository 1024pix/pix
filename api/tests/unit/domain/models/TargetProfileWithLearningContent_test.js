...
describe('Unit | Domain | Models | TargetProfileWithLearningContent', function () {
  ...
  describe('getTubeIdOfSkill()', function () {
    ...
    beforeEach(function () {
      ...
      targetProfile = domainBuilder.buildTargetProfileWithLearningContent({
        skills: [skillNotInTube, skillInTube],
        tubes: [tube1, tube2],
      });
    });
  ...
  describe('getCompetenceIdOfSkill()', function () {
    ...
    beforeEach(function () {
      ...
      const tube1 = domainBuilder.buildTargetedTube({
        id: 'tube1',
        skills: [skillNotInCompetence],
        competenceId: 'otherCompId',
      });
      const tube2 = domainBuilder.buildTargetedTube({
        id: 'tube2',
        skills: [skillInCompetence],
        competenceId: expectedCompetenceId,
      });
      targetProfile = domainBuilder.buildTargetProfileWithLearningContent({
        skills: [skillNotInCompetence, skillInCompetence],
        tubes: [tube1, tube2],
      });
    });
  ...
  describe('getAreaOfCompetence()', function () {
    ...
    beforeEach(function () {
      ...
      targetProfile = domainBuilder.buildTargetProfileWithLearningContent({
        competences: [competenceNotInArea, competenceInArea],
        areas: [expectedArea, area1],
      });
    });
  ...
  describe('getKnowledgeElementsGroupedByCompetence()', function () {
    it('should return knowledge elements of targeted skill by targeted competence id', function () {
      ...
      const knowledgeElement2_1 = domainBuilder.buildKnowledgeElement({
        competenceId: 'recCompetence2',
        skillId: 'recSkill2_1',
      });
      const knowledgeElement2_2 = domainBuilder.buildKnowledgeElement({
        competenceId: 'recCompetence2',
        skillId: 'recSkill2_2',
      });
  ...
  describe('countValidatedTargetedKnowledgeElementsByCompetence()', function () {
    it('should return validated knowledge elements count by targeted competences', function () {
      ...
      const validatedCountByCompetence =
        targetProfile.countValidatedTargetedKnowledgeElementsByCompetence(knowledgeElements);
    ...
    it('should proceed counting knowledgeElement within actual competenceId and not based on the declared one', function () {
      ...
      // when
      const knowledgeElementsByCompetence =
        targetProfile.countValidatedTargetedKnowledgeElementsByCompetence(knowledgeElements);
  ...
  describe('get#maxSkillDifficulty', function () {
    it('should highest difficulty of target profile skills', function () {
      ...
      const targetProfile = domainBuilder.buildTargetProfileWithLearningContent({
        skills: [mostDifficultSkill, lessDifficultSkill],
      });
  ...
  describe('getThresholdBoundariesFromStage()', function () {
    it('should return skill count boundary for the given stage id', function () {
      ...
      // then
      expect(skillCountBoundaries).to.deep.equal([{ id: 'stage2', from: 40, to: 79 }]);
    });
    ...
    it('should return total skills count in "to" if it’s the last stage', function () {
      ...
      // then
      expect(skillCountBoundaries).to.deep.equal([{ id: 'stage3', from: 80, to: 100 }]);
    });
  });
});
