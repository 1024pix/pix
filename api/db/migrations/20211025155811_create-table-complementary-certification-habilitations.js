exports.up = async function(knex) {
  ...
  const complementaryCertificationHabilitations = grantedAccreditations.map(({ id, accreditationId, certificationCenterId, createdAt }) => {
    return {
      id,
  ...
  if (complementaryCertificationHabilitations.length > 0) {
    ...
    await knex.raw(`ALTER SEQUENCE "complementary-certification-habilitations_id_seq" RESTART WITH ${idForNextInsertion}`);
  }
};
